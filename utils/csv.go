...
// 定义一个 Getter 方法
func (c *CloudflareIPData) LossRate() float32 {
	return c.lossRate
}
...
func ExportCsv(data []CloudflareIPData) {
	...
	_, err = fp.Write(bom)
	if err != nil {
		fmt.Println("写入 BOM 失败:", err)
		return
	}

	w := csv.NewWriter(fp) //创建一个新的写入文件流
	_ = w.Write([]string{"IP 地址", "已发送", "已接收", "丢包率", "平均延迟", "下载速度 (MB/s)", "端口", "备注"})
...
func (s DownloadSpeedSet) Print() {
	...
	headFormat := "%-16s%-5s%-5s%-5s%-6s%-11s%-7s%-16s\n"
	dataFormat := "%-18s%-8s%-8s%-8s%-10s%-15s%-7s%-16s\n"
	for i := 0; i < PrintNum; i++ { // 如果要输出的 IP 中包含 IPv6，那么就需要调整一下间隔
		if len(dateString[i][0]) > 15 {
			headFormat = "%-40s%-5s%-5s%-5s%-6s%-11s%-7s%-16s\n"
			dataFormat = "%-42s%-8s%-8s%-8s%-10s%-15s%-7s%-16s\n"
			break
		}
	}
	fmt.Printf(headFormat, "IP 地址", "已发送", "已接收", "丢包率", "平均延迟", "下载速度 (MB/s)", "端口", "备注")
